				 ...
				 const SampleDiffs& maternal_maternal_de_tests)
{
		 ...
		 ++pp_itr,++pm_itr,++mp_itr,++mm_itr)
	{
        ...
        const char* paternal_paternal_status;
		const char* paternal_maternal_status;
		const char* maternal_paternal_status;
		const char* maternal_maternal_status;
						 ...
						 const FPKMTrackingTable& tracking)
{
	...
	for (FPKMTrackingTable::const_iterator itr = tracking.begin(); itr != tracking.end(); ++itr)
	{
		...
		for (size_t i = 0; i < paternal_fpkms.size(); ++i)
		{
			...
			double paternal_fpkm_conf_hi = paternal_fpkm + 2.0 * paternal_std_dev;
			double paternal_fpkm_conf_lo = max(0.0, paternal_fpkm - 2.0 * paternal_std_dev);
			...
			double maternal_fpkm_conf_hi = maternal_fpkm + 2.0 * maternal_std_dev;
			double maternal_fpkm_conf_lo = max(0.0, maternal_fpkm - 2.0 * maternal_std_dev);
...
//    else
	...
	{
		allele_test_launcher = shared_ptr<AlleleTestLauncher>(new AlleleTestLauncher(bundle_factories.size(), NULL, &tracking, samples_are_time_series, &p_bar));
	}
	...
	if (corr_bias || corr_multi) // Only run initial estimation if correcting bias or multi-reads
	{
		while (1) 
		{
			...
			//else
			//{
            //allele_test_launcher = shared_ptr<AlleleTestLauncher>(new AlleleTestLauncher((int)bundle_factories.size(), NULL, &paternal_tracking, &maternal_tracking, samples_are_time_series, &p_bar));
            //}
...
//    }
    ...
    int N = (int)sam_hit_filename_lists.size(); //number of conditions
    if(!allele_specific_differential)
	{
	...
	if(!allele_specific_differential)
		...
		allele_test_launcher = shared_ptr<AlleleTestLauncher>(new AlleleTestLauncher(bundle_factories.size(), &alleleTests, &tracking, samples_are_time_series, &p_bar));

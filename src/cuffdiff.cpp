...
boost::shared_ptr<AlleleTestLauncher> allele_test_launcher;
...
//    else
	...
	{
		allele_test_launcher = boost::shared_ptr<AlleleTestLauncher>(new AlleleTestLauncher(bundle_factories.size(), NULL, &tracking, samples_are_time_series, &p_bar));
	}
	...
	if (model_mle_error || corr_bias || corr_multi) // Only run initial estimation if correcting bias or multi-reads
	{
		while (1) 
		{
			...
			{
				boost::shared_ptr<vector<boost::shared_ptr<SampleAlleleAbundances> > > alleleAbundances(new vector<boost::shared_ptr<SampleAlleleAbundances> >());

...

{
	...
	FPKMContext paternal_context(
		ab.num_paternal_fragments(),
 	...
 	FPKMContext maternal_context(
		ab.num_maternal_fragments(),
    ...
    if (sample_index < paternal_fpkms.size())
    {
        ...
        FPKMContext& paternal_existing = paternal_fpkms[sample_index];
        FPKMContext& maternal_existing = maternal_fpkms[sample_index];
        paternal_existing.FPKM += paternal_context.FPKM;
        maternal_existing.FPKM += maternal_context.FPKM;
        paternal_existing.count_mean += paternal_context.count_mean;
        maternal_existing.count_mean += maternal_context.count_mean;
        paternal_existing.FPKM_variance += paternal_context.FPKM_variance;
        maternal_existing.FPKM_variance += maternal_context.FPKM_variance;

        if (paternal_existing.status == NUMERIC_FAIL || paternal_context.status == NUMERIC_FAIL)
            paternal_existing.status = NUMERIC_FAIL;
        else
            paternal_existing.status = NUMERIC_OK;

        if (maternal_existing.status == NUMERIC_FAIL || maternal_context.status == NUMERIC_FAIL)
            maternal_existing.status = NUMERIC_FAIL;
        else
            maternal_existing.status = NUMERIC_OK;   
    }
    else
    {
        paternal_fpkms.push_back(paternal_context);
        maternal_fpkms.push_back(maternal_context);
    }
}
